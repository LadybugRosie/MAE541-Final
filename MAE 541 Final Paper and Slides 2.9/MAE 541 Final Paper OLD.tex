...
Recurrent neural networks (RNNs) exhibit a variety of dynamical behaviors arising from the interaction between nonlinear neural activation and recurrent feedback. In fixed-weight RNNs, dynamics are strongly governed by the eigenvalue spectrum of the weight matrix, with critical phase transitions separating stable and chaotic regimes. Studies show that most neuronal and biological systems have parameter values such that they lie at the critical boundary between stability and chaos, at the so-called "edge of chaos" \hl{CITATION}. It is hypothesized that being in the "edge of chaos" allows such systems to use high-dimensional transient behavior while preserving the stability of dynamics, ultimately allowing for more complex and adaptive behavior \hl{CITATION}.
\vspace{0.5cm}
\\
A way to look at computation and complexity in a recurrent neural network is through the concept of reservoir computing, where the transient behavior of neural networks with pseudo-random weights is harnessed for computation, such as signal reconstruction. \hl{MAYBE EXPLAIN RESERVOIR COMPUTING BETTER?} Central to the complexity of understanding how criticality and parameter regimes influence the ability for reservoir computing techniques to properly reconstruct signals is the ability for biological neural systems to learn and continuously modify synaptic strengths through learning rules, so the computational power and topology of the dynamical phase space can change dramatically over time. Of particular interest are local activity-dependent learning rules based on the Hebbian-type learning. In this paper, we study how recurrent neural networks with Oja-type plasticity, a modification of Hebbian learning, impacts the dynamical behavior of the system and alters signal reconstruction capabilities of the system. We also explore how structured inputs to the network, such as sinusoidal driving, can impact the weight learning process and effective dimensionality of the system.
...
\begin{equation}
\tau \dot{\bf x} = -\bf x+f\left(W \cdot \bf x+\bf b\right)
\end{equation}
\\
...
\end{equation*}
\\
for some vector $\bf x \in \mathbb{R}^n$. In this setting, memory and computation are usually studied through the evolving neural state \(\mathbf{x}(t)\), while the matrix \(W\) defines the fixed recurrent architecture.

\subsection{Learning in Recurrent Neural Networks}
In the fixed-weight setting, the state \({\bf x}(t)\) evolves through a fixed recurrent architecture \(W\). Introducing learning makes the weights dynamic, so the system evolves in the enlarged state space
...
long-term behavior of the network \hl{CITATION}. We can also add a forgetting term parametrized by $\gamma$ to get
...
Motivated by the use of random weight initializations in reservoir computing, we utilized Girko's circular law, which allows us to estimate the spectrum of random matrices, to avoid computationally infeasible eigenvalue calculations for large random matrices.
\vspace{0.5cm}
\\
...
\]
\\
Furthermore, let \(\lambda_1,\dots,\lambda_n \in \mathbb{C}\) denote the eigenvalues of \(W^{(n)}\), and define the empirical spectral measure
...
\]
\\
is the uniform probability measure on the disk of radius \(g\) in \(\mathbb{C}\). For a sufficiently large system of $n$ neurons with weights sampled from $ \mathcal{N}(0, \frac{g^2}{n})$, Girko's Circular Law predicts that the eigenvalues of the neural weight matrix distribute uniformly on the disk
\[
...
integrates its total recurrent input. An alternative formulation that is formally equivalent to the model above is
...
To understand how system dynamics change in the non-autonomous input-driven case with changing weights, we first look at the autonomous case where weights are frozen and $\bf b = 0$, so there is no input. Then the simplified system of equations is:

\begin{equation} \label{4.1}
    \tau \dot{\bf{x}}  = -\bf{x} +  \tanh(W {\bf x})
\end{equation}
\\

All activity is generated internally by the recurrent network. Note that for any neuron $x_i$,
...
\vspace{0.5cm}
\\
Using \eqref{4.1} to solve for equilibrium, we find that there are two possible types of equilibrium, the trivial equilibrium at the origin $\bf x^*=0$ and nontrivial equilibria that exist when the following system of equations is satisfied
...
\vspace{0.5cm}
\\
In the one-dimensional or single neuron case, a pair of nontrivial equilibria only exist when the slope of $\tanh(W_{11}x_1)-x_1$ is positive at the origin \hl{CITATION}. Thus, in the n-dimensional case, a pair of nontrivial equilibrium generically exist only if there exists an eigenvector $v$ of $J(0)$ such that the eigenvalue $\mu$ of $J(0)$ corresponding to that eigenvalue is positive. \hl{CHECK THIS ARGUMENT ABOVE DAVID} Linearizing around the origin yields the Jacobian
...
\vspace{0.5cm}
\\
The Jacobian at a nontrivial equilibrium $\bf x^*$ is given by

\begin{equation}
J(x^*) = \frac{1}{\tau}(DW - \text{Id})
\end{equation}

Here $D = \text{diag} (1 - (x_i^*)^2)$. For $||x|| \approx 1$, $J(x) = -\frac{\text{Id}}{\tau}$, so nontrivial equilibrium of \eqref{4.1} with $||x^*|| \approx 1$ will be locally asymptotically stable and nontrivial equilibrium with $||x^*|| \approx 0$ will be unstable due to the previous analysis of the stability of the origin. Since increasing the eigenvalues of $W$ can saturate $D$, the effective eigenvalues of $DW - I$ can be reduced, implying there is a critical region for the eigenvalues of $W$ where we can have a stable nontrivial equilibrium.
...
\begin{equation}
    C(W) = \frac{1}{T_{d}} \int_{0}^{T_d}\bf{x}(t; W)\bf{x}(t;W)^T \, dt
\end{equation}
\\
...
\end{equation}
\\
For $\bf{x}(t) = \bf{x}$, that is constant neural activity, the above equation reduces to 
...
\vspace{0.5cm}
\\
\hl{Should there be a $||r||^2$ term in the denominator?, multiplying $\eta$} If we additionally assume that under saturation, the phases of the neural activities approximately align, then $\cos(\theta_i - \theta_j) \approx 1$ and so
...
Applying Girko's law to $D(t)W$ motivates, akin to a mean-field approximation, what we denote
...
\vspace{0.5cm}
\\
Using $g_{\text{eff}}$ as a proxy for gain, we recover the stability and fixed point analysis results from Section~$4.1$ and $4.2$ for the non-chaotic regime ($g_{\text{eff}} \leq 1$), with the difference that now we have periodic orbits instead of fixed points.
...
\end{equation}
\begin{equation}
    \dot{W}_{ij} = \eta\,x_i(x_j - x_i W_{ij}) - \gamma W_{ij}
\end{equation}
\\
...
of the network's response \hl{CITATION}. 
...
For each $g_0$, we keep the weights fixed (i.e., $\eta, \gamma = 0$) and fit a ridge-regression
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{signal_comparison/correlation_rank_by_drive.png}
...
\hl{CLARIFY THIS SECTION AND WHY THE 26 IS AN ARTIFACT}For $g < 1$, the stable regime, white noise keeps $\mathrm{rank}(\hat{C})\approx 26 $ and
...
generic \hl{EXPLAIN THIS MORE REASONABLY and BETTER GRAPHICS}.  Quasi-periodic drive with three incommensurate frequencies collapses $\rho(W)$
